...
export function AppHeader({ initialUser, userRole }: AppHeaderProps) {
    ...
    useEffect(() => {
        ...
        const checkUser = async () => {
            ...
            // Only fetch role if not already provided and user exists
            if (user && !userRole && !role) {
                try {
                    ...
                    console.error('[AppHeader] Exception fetching profile:', err)
                    setRole('client')
                }
            } else if (userRole) {
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${hideOnLargeScreens} ${isScrolled || !isLandingPage ? 'bg-black/90 backdrop-blur-xl border-b border-white/5 py-2' : 'bg-transparent py-4'
            }`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        {isLandingPage && !user ? (
                            ...
                            authNavLinks.map((link) => (
                                <Link
                                    key={link.name}
                                    href={link.href}
                                    className={`relative px-4 py-2 group flex items-center gap-2 ${pathname === link.href ? 'text-white' : 'text-white/50 hover:text-white'
                                        }`}
                                >
                ...
                {/* Mobile Menu */}
                <div className={`md:hidden absolute top-full left-0 right-0 bg-black/95 backdrop-blur-xl border-b border-white/5 transition-all duration-500 ${isMenuOpen ? 'opacity-100 translate-y-0 max-h-[85vh] overflow-y-auto' : 'opacity-0 -translate-y-4 max-h-0 overflow-hidden'
                    }`}>
                            ...
                            <>
                                {landingNavLinks.map((link, index) => (
                                    <a
                                        ...
                                        className={`text-white/70 hover:text-white hover:bg-white/5 px-6 py-3 transition-all duration-300 transform ${isMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-4 opacity-0'
                                            }`}
                            ...
                            <>
                                {authNavLinks.map((link, index) => (
                                    <Link
                                        ...
                                        className={`flex items-center gap-3 px-6 py-3 transition-all duration-300 ${pathname === link.href ? 'text-white bg-[#df2531]/10 border-l-2 border-[#df2531]' : 'text-white/70 hover:text-white hover:bg-white/5'
                                            }`}
                                ...
                                <Link
                                    ...
                                    onClick={() => setIsMenuOpen(false)}
                                    className={`flex items-center gap-3 px-6 py-3 transition-colors ${pathname === '/dashboard/profile' ? 'text-white bg-[#df2531]/10 border-l-2 border-[#df2531]' : 'text-white/70 hover:text-white hover:bg-white/5'
                                        }`}
                                >
                                ...
                                <Link
                                    ...
                                    onClick={() => setIsMenuOpen(false)}
                                    className={`flex items-center gap-3 px-6 py-3 transition-colors ${pathname === '/dashboard/settings' ? 'text-white bg-[#df2531]/10 border-l-2 border-[#df2531]' : 'text-white/70 hover:text-white hover:bg-white/5'
                                        }`}
                                >
